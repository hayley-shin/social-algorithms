...
def load_name_data(female_file='female_names.tsv.gz', male_file='male_names.tsv.gz'):
    ...
    # read tsv.gz file and add counts
    def read_file(filepath, gender_idx):
        # skip the header row
        with gzip.open(filepath, 'rt', encoding='utf-8') as f:
            ...
            for row in f:
                ...
                # split each value (name, count, year)
                values = row.split('\t')
                # lowercase them (names should be case-insensitive
                name = values[0].strip().lower()

                try:
                    count = int(values[1])
                # (just in case) skip if invalid number or doesn't exist
                except (ValueError, IndexError):
                    continue
                
                if name not in name_counts:
                    name_counts[name] = [0, 0]
                name_counts[name][gender_idx] += count
    
    # load female and male name data and add counts
    read_file(female_file, 0)
    read_file(male_file, 1)
...
def infer_gender(display_name, name_data, threshold=0.6):
    ...
    # return 'U' -> if empty name or invalid
    if not display_name or not isinstance(display_name, str):
        return 'U'
    
    # keep letters, spaces, ', -
    name_cleaned = re.sub(r"[^A-Za-z\s'\-]", " ", display_name)
    name_cleaned = re.sub(r"\s+", " ", name_cleaned).strip()
    # return 'U' -> if empty name
    if not name_cleaned:
        return 'U'
    
    # split the display name and remove common prefix
    name_splitted = name_cleaned.split()
    prefixes = {'mr', 'mrs', 'ms', 'miss', 'dr', 'prof', 'sen', 'senator', 'rep', 'gov', 'aunt', 'uncle'}
    while name_splitted:
        tmp = name_splitted[0].lower().strip(".")
        if tmp in prefixes:
    ...
    # assumed first name
    first_name = name_splitted[0].strip("'-").lower()

    # return 'U' -> if name is 1 letter or including non-alphabet letter
    if len(first_name) < 2 or not first_name.isalpha():
        return 'U'
    
    # bring female and male count number
    name_counts = name_data.get(first_name)
    # return 'U' -> if first name is not in name_data
    if not name_counts:
        return 'U'
    female_count = name_counts[0]
    male_count = name_counts[1]
    total_count = female_count + male_count
    # return 'U' -> if total name count is 0
    if total_count == 0:
        return 'U'
    
    # assign 'F', 'M', 'U'
    female_percent = female_count / total_count
    if female_percent >= threshold:
        return 'F'
    elif female_percent <= (1-threshold):
        return 'M'
    else:
        return 'U'
